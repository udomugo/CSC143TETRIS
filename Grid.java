...
public class Grid {
	...
	public void checkRows() {
		...
		for ( int row = HEIGHT - 1; row > 0; row--) {
			...
			if (rowIsFull) {
				for ( int col = 0; col < WIDTH; col++) {
					this.set(cascadeRow, col, EMPTY);
				}
				for ( cascadeRow--; cascadeRow > 0; cascadeRow--) {
					for ( int col = 0; col < WIDTH; col++) {
						if (this.isSet(cascadeRow, col)) {
							board[cascadeRow + 1][col].setColor(board[cascadeRow][col].getColor());
							board[cascadeRow][col].setColor(EMPTY);
						}
					}
				}
			}
